...
2047, 2831, 3500, 3940, 4078, 3906, 3471, 2831, 2047, 1263, 594, 154, 
16, 188, 723, 1528
};


void audio_tone(int duration, uint32_t tone) {
    static uint32_t index = 0; // Keeps track of the current position in the sine table
    for (int i = 0; i < duration; i++) {
        VDAC_Channel1OutputSet(VDAC0, (sineTable[index]));
        // printf("sineTable[%u] = %u\n", index, (sineTable[index]));
        
        // Use the delay2 parameter in the loop
        for (uint32_t j = 0; j < tone; j++) { 
            __NOP();	
        }
...
void rail_callback(RAIL_Handle_t rail, RAIL_Events_t events)
{
	...
	if (events & RAIL_EVENT_RX_FIFO_ALMOST_FULL) {
		...
		VDAC_Channel1OutputSet(VDAC0, audio_out * 20); // ARDF OH2BTG
#endif
...
int start_tx_dsp(RAIL_Handle_t rail)  // OH2BTG here TX is switched ON
{
	(void)rail;
...
#ifdef RX_EN_PIN  // RX TX ant switch control
	//GPIO_PinOutClear(RX_EN_PORT, RX_EN_PIN);
	//GPIO_PinOutSet(TX_EN_PORT, TX_EN_PIN);	
	//GPIO_PinOutClear(F11_EN_PORT, F11_EN_PIN);  // OH2BTG nastojen testausta
	//GPIO_PinOutSet(F12_EN_PORT, F12_EN_PIN);    // OH2BTG nastojen testausta
#endif
...
int read_batt_adc(){
	setup_adc();
	ADC_Start(ADC0, adcStartSingle);
	readADC = ADC_DataSingleGet(ADC0);
	//uint32_t adcValue = readADC;
	//printf("ADC Value: %lu\n", readADC); // Print ADC result
	return readADC;
}
...
void setup_adc(void)
{
	...
	/*
	ADC_InitSingle(ADC0, &(const ADC_InitSingle_TypeDef) { // ARDF tämä tarvitaan TX modessa
		.prsSel = adcPRSSELCh0,
		.acqTime = adcAcqTime4,
		.reference = adcRef1V25,
		.resolution = adcResOVS,
		.posSel = MIC_APORT, // ARDF testing was MIC_APORT kokeile tätä
		.negSel = adcNegSelVSS,
		.diff = 0,
		.prsEnable = 0,
		.leftAdjust = 1,
	...
	ADC_InitSingle(ADC0, &(const ADC_InitSingle_TypeDef) { // ARDF for battery meter testing
	.reference = adcRefVDD,          // Use AVDD as reference (e.g., 3.3 V)
    .resolution = adcRes12Bit,       // 12-bit resolution
    .acqTime = adcAcqTime16,          // Acquisition time
    .posSel = BAT_APORT,   			// Select input channel
    .negSel = adcNegSelVSS,   
    });
}
