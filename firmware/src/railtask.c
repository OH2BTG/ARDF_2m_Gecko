...
int16_t RSSI_read = 0;
	void ARDF_RSSI (){
	int16_t rssi = RAIL_GetRssi(rail, false);
	//printf("Current RSSI: %d dBm\n", rssi/4);
	RSSI_read = rssi;
	//printf(" RSSI_read: %d dBm\n", RSSI_read);
	return rssi/4; 
	}
...
void railtask_init_radio(void)
{
	...
	int16_t rssi = RAIL_GetRssi(rail, false); // OH2BTG
...
void railtask_main(void *arg)
{
	...
	for(;;) {
		...
		//if (keyed && mode >= MODE_USB && mode <= MODE_CWL) {  // ARDF CW removed
		//	frequency += offset;
		//}
