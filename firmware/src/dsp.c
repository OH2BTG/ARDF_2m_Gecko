...
// Declare these as extern in the header
//extern float agc_attack;
//extern float agc_decay;
...
#define ZOOM_FACTOR 4  // ARDF Adjust to zoom in (higher values zoom more)
#define DISPLAY_WIDTH 128  // ARDF Width of the waterfall display
#define DISPLAY_BINS (FFT_BIN2 - FFT_BIN1) // ARDF Number of bins selected

float global_amp = 1000;  // ARDF Default value
float global_agc_amp = 50000; // ARDF Default value
float global_agc_decay;
float global_o_disp;
float agc_amp;
int agc_normal = 0;
...
void demod_store(struct demod *ds, iq_in_t *in, unsigned len)
{
	...
	if((ds->smeter_count += len) >= 0x4000) {  // chatgpt given idea orig value 0x4000
...
/*static inline*/ void demod_am(struct demod *ds, iq_in_t *in, float *out, unsigned len)
{
	...
	for (i = 0; i < len; i+=2) {
		...
		o += (ai >= aq) ? (ai + aq * beta) : (aq + ai * beta);
		out[i/2] = o;  // ARDF orig
		//out[i/2] = o * 2.0f; // ARDF lisää audio volumea ei poista säröä
		global_o_disp = o;  // ARDF
	}
}
...
// Biquad coefficients for CW OH2BTG
...
static const struct biquad_coeff biquads_cw[BIQUADS_SSB_N] = {
      ...
      {-1.774495E+00f,7.876780E-01f,5.918198E-08f,1.183640E-07f,5.918198E-08f},
      {-1.796139E+00f,8.109451E-01f,1.000000E+00f,2.000000E+00f,1.000000E+00f},
      {-1.849351E+00f,8.687560E-01f,1.000000E+00f,2.000000E+00f,1.000000E+00f}
};
...
void demod_ssb(struct demod *ds, iq_in_t *in, float *out, unsigned len)
{
	...
	const struct biquad_coeff *filter =
		//(ds->mode == MODE_CWU || ds->mode == MODE_CWL) // ARDF removed
		(ds->mode == MODE_USB || ds->mode == MODE_USB) // ARDF removed
 ...
 */
/*static inline*/
 void demod_audio_filter(struct demod *ds, float *buf, unsigned len)
{
	unsigned i;
	 ...
	 * runs at a lower sample rate. OH2BTG agc attack = 0.1f agc_decay = 0.01f;*/
	float agc_attack = 0.1f, agc_decay = 0.1f; // 0.1 agc_attack ARDF new value date 14142024 const removed

	float agc_amp = ds->agc_amp;  //is updated to track the signal amplitude.

	//float gain = 1.0f / (ds->agc_amp + 1e-6f); // ARDF and ChatGPT avoid divide-by-zero
	...
	agc_amp = global_agc_amp;	
	} 
	// ARDF adjusting agc based to RSSI 15032025 01052025 08072025
	//float d = global_agc_amp - agc_amp; // ARDF 26.03.2025
	//float d = amp; // ARDF
	//global_amp = amp; // ARDF 08072025 disabled
	global_amp = agc_amp; // ARDF 08072025 test
	float d = amp - agc_amp; // original AGC function
...
int dsp_fast_rx(iq_in_t *in, int in_len, audio_out_t *out, int out_len)
{
	...
	//case MODE_CWL:
		...
		break;
	default: // default AM mode for ARDF
		demod_am(&demodstate, in, audio, in_len);
		break;
	}
...
static void mod_process_audio(struct modstate *m, audio_in_t *in, float *out, unsigned len)
{
	...
	const float agc_attack = 0.1f, agc_decay = 0.0005f;
	// const float agc_attack = 0.1f, agc_decay = 0.002f; OH2BTG
...
void dsp_update_params(void)
{
	...
	float minGain = 0.001f;     // Minimum gain factor
	float maxGain = 100.0f;    // Maximum gain factor, adjust to your application needs
	// Scale volume logarithmically to provide smoother perception of volume changes
	demodstate.audiogain = minGain + (maxGain - minGain) * (log2f((float)vola + 1) / log2f(50.0f)); // pieni luku isot portaat
	//float maxGain = 3000.0f; // OH2BTG good value
	//demodstate.audiogain = ((float)vola / 50.0f) * maxGain;  // 01102024 good OH2BTG Good volume adjust
...
static void calculate_waterfall_line(unsigned sbp)
{
  ...
  // Rescale FFT bins to fit display width
  //  for (i = 0; i < DISPLAY_WIDTH; i++) {
   //     unsigned bin = FFT_BIN1 + (i * DISPLAY_BINS) / (DISPLAY_WIDTH * ZOOM_FACTOR);
   //    if (bin >= FFT_BIN2) bin = FFT_BIN2 - 1; // Clamp to prevent overflow
	...
	for(i=FFT_BIN1;i<FFT_BIN2;i++) {
	 ...
	 display_ev.waterfall_line = 1;  // OH2BTG
	xSemaphoreGive(display_sem);
}
